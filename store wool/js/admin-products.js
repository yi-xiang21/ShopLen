		...
		if (
          ...
          const firstVariant = product.variants[0];
			if (firstVariant.images && Array.isArray(firstVariant.images) && firstVariant.images.length > 0) {
				imageUrl = firstVariant.images[0].imageUrl || firstVariant.images[0] || imageUrl;
...
async function fetchProducts(keyword = '', catId = '') {
  ...
  function addVariant(variant = null) {
    ...
    variantDiv.innerHTML = `
			...
			<div class="variant-fields">
                <label>Màu:</label>
                <input type="text" class="variantColor" placeholder="red" value="${variant ? (variant.color || '') : ''}">

                <label>Kích cỡ:</label>
                <input type="text" class="variantSize" placeholder="100gr" value="${variant ? (variant.size || '') : ''}">

                <label>Chất lượng:</label>
                <input type="text" class="variantMaterial" placeholder="Cotton" value="${variant ? (variant.material || '') : ''}">
    ...
    function createVariantImageRow(imageUrl = '', removable = true) {
      ...
      if (removable) {
        ...
        row.innerHTML = `
					<input type="file" class="variant-image-input" accept="image/*">
					${imageUrl ? `<img src="${imageUrl}" style="max-width: 80px; max-height: 80px;" class="variant-image-preview">` : ''}
					<input type="hidden" class="variant-image-url" value="${imageUrl || ''}">
				`;
      }
    ...
    if (images.length > 0) {
      images.forEach((img, idx) => {
        const imgUrl = typeof img === 'string' ? img : (img.imageUrl || img.imagePath || '');
  ...
  async function fetchCategories() {
    try {
      // Tạm thời, sau này sẽ có API /categories
      const categories = [
        { id: 1, name: 'Len' },
        { id: 2, name: 'Công cụ' },
        { id: 3, name: 'Workshop' },
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          variantItems.forEach((item, variantIndex) => {
            const color = item.querySelector('.variantColor')?.value.trim() || '';
            ...
            imageInputs.forEach((input, imgIdx) => {
              ...
              const existingUrl = input.parentElement.querySelector('.variant-image-url')?.value;

              if (file) {
                // Upload file mới - append với fieldname có index biến thể và index hình
                formData.append(`variantImages_${variantIndex}_${imgIdx}`, file);
            ...
            if (color || size || material || imageInputs.length > 0 || imageUrls.length > 0) {
                const variantData = {
                    color,
                    size,
                    material,
